...
A `SmartTable` automatically gathers warnings from the filling functions and display them as note of the jamovi table. We can try this by producing a warning in the `run_main_coefficients` function.
...
Now you can decide what to do with the outcome variable. You can stop the processing (in case `error` is not `FALSE`), you can ignore it, or pass it to a table as we did before in \@ref(codewarnings). The same goes with warnings and messages.
