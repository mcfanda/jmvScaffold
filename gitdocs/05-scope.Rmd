...
Let's make our module a bit more complex adding one option in the GUI. It allows the user to show the `means` array, that we now want hidden by default.
...
## Showing table on option change

In this example, we want the `means` array to be shown only when the user selects `Show means` in the GUI. We need to modify the `.r.yaml` file as follows:
...
Now we need to be sure that the `runner$run_additional_effects()` returns something only if there is a significant result in the coefficients.
