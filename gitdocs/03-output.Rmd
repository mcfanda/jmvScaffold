...
You can go a long way off without the this section. One can add tables ad define a function or a procedure to fill each table passing the results data to the `$runTable` function. However, when your module gets more complex, you want to give a structure to the calls that fill the tables, so you can forget about them and focus on the R programming. Here comes the last method `jmvScaffold` offers to populate the jamovi tables. 
