...
a<-"arrayname_!_tablename"
b<-"arrayname_mykey_tablename"

ll<-stringr::str_split(b,"_")[[1]]
alist<-list(ll[1])
for (l in ll[-1])
      alist[[length(alist)+1]]<-paste(alist[[length(alist)]],l,sep ="_")
r<-unlist(alist)   
unique(c(stringr::str_replace_all(r,"_mykey_","_!_"),r))
a1<-gsub("mykey$","!",a)
